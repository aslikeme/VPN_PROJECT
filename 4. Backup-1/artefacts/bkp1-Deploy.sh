...
scp -i /home/$bckpUser/.ssh/id_rsa $caUser@$caIp:~/easy-rsa/pki/issued/$bkp1Addr.crt /opt/node_exporter/ || { echo "Что-то пошло не так! Копирование не удалось!"; exit 1; } # перенос сертификата для сервера node_exporter на bkp1-машине
scp -i /home/$bckpUser/.ssh/id_rsa $caUser@$caIp:~/easy-rsa/pki/private/$bkp1Addr.key /opt/node_exporter/ || { echo "Что-то пошло не так! Копирование не удалось!"; exit 1; } # перенос ключа сервера node_exporter на bkp1-машине
